sap.ui.define([
  ...
  return BaseController.extend("sap.ui.demo.cart.controller.Manufacturer", {
              ...
              this.ODataHelper.callOData(this.getOwnerComponent().getModel(),
                ...
                .then(function(oData) {
                  ...
                  for (var i = 0; i < oData.results.length; i++) {
                    ...
                    debugger;
                    that.getView().getModel("local").setProperty("/groupSelected/GroupCode", group[0].groupCode);
                    that.getView().getModel("local").setProperty("/groupSelected/GroupId", group[0].id);
                    that.getView().getModel("local").setProperty("/groupSelected/ManuId", oData.results[i].id);
                    var selectedGroups = that.getView().getModel("local").getProperty("/groupSelected");
                    groupID.push(selectedGroups);
                  }
                })
                .catch(function(oError) {
            ...
            this.ODataHelper.callOData(this.getOwnerComponent().getModel(),
              ...
              .then(function(oData) {
                  ...
                  function recursion() {
                    ...
                    // var status = that.updateGroupDetails( groupId, count, manufactureID);
                    // if (status === "true") {
                    //   i++;
                    //   recursion();
                    // }
                    var manuGroup = that.getView().getModel("local").getProperty("/ManuGroup");
                    if (groupId[i] !== "") {
                      manuGroup.ManufacturerId = manufactureID;
                      manuGroup.GroupId = groupId[count];
                      manuGroup.ChangedBy = "";
                      manuGroup.ChangedOn = "";
                      manuGroup.CreatedBy = "";
                      manuGroup.CreatedOn = "";
                      that.ODataHelper.callOData(this.getOwnerComponent().getModel(),
                          "/ManuGroups", "POST", {}, manuGroup, that)
                        .then(function(oData) {
    ...
    groupCodeCheck: function(oEvent) {
      ...
      changeCheck = 'true';
      // var newValue = input_source.getParameter("newValue");
      // var  oldValue = input_source.getParameter("value");
